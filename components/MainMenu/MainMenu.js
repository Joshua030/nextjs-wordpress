...
const MainMenu = ({ items, callToActionLabel, callToActionDestinaton}) => {
      ...
      <div className='flex flex-1 justify-end'>
        {items?.map(item => (
          <div key={item.id} className='hover:bg-slate-700 cursor-pointer relative group'>
            <div>
              <Link className='p-5 block' href={item.destination}>{item.label}</Link>
            </div>
        ...
        <ButtonLink destination={callToActionDestinaton} label={callToActionLabel} />
        </div>
      </div>
    </div>
  )
